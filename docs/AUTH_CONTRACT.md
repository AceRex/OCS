...
This document specifies the interface contract between the **OCS Desktop Application** and the **Hosted OCS Authentication Web Portal** (`auth.churchocs.com`).
...
sequenceDiagram
    ...
    participant WebAuth as Hosted Auth Portal (auth.churchocs.com)
    ...
    Browser->>WebAuth: GET /login?state=...&app=desktop&redirect_uri=ocs%3A%2F%2Fauth-callback
...
### Request Format
```
GET https://auth.churchocs.com/login?state={STATE}&app=desktop&redirect_uri={REDIRECT_URI}
```
...
| `redirect_uri` | `string` | **Yes** | URL-encoded custom scheme callback (`ocs%3A%2F%2Fauth-callback`). |
