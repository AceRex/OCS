...
async function runTests() {
  const currentContext = { bookIndex: 42, chapter: 3 }; // Current: John (index 42), Chapter 3

  const testPhrases = [
    { phrase: "go to vass 20", expectedVerse: 20 },
    { phrase: "good to us 20", expectedVerse: 20 },
    { phrase: "jump to 20", expectedVerse: 20 },
    { phrase: "go to 20", expectedVerse: 20 },
    { phrase: "skip to verse 20", expectedVerse: 20 },
    { phrase: "go to verse twenty", expectedVerse: 20 },
    { phrase: "vass 20", expectedVerse: 20 },
    { phrase: "verse 20", expectedVerse: 20 },
    { phrase: "read verse 20", expectedVerse: 20 },
    { phrase: "move to 20", expectedVerse: 20 },
    { phrase: "turn to 20", expectedVerse: 20 },
  ];

  let passed = 0;
  for (const tc of testPhrases) {
    const shape = matchReferenceShape(tc.phrase);
    assert(shape.shortContext || shape.complete, `Shape failed for: "${tc.phrase}"`);

    const res = await smartBibleMatch(tc.phrase, books, null, currentContext, { allowPass2: true, requireShape: false });
    assert(res, `smartBibleMatch returned null for: "${tc.phrase}"`);
    assert.strictEqual(res.bookIndex, 42, `Expected bookIndex 42 for "${tc.phrase}", got ${res.bookIndex}`);
    assert.strictEqual(res.chapter, 3, `Expected chapter 3 for "${tc.phrase}", got ${res.chapter}`);
    assert.strictEqual(res.startVerse, tc.expectedVerse, `Expected verse ${tc.expectedVerse} for "${tc.phrase}", got ${res.startVerse}`);
    passed++;
    console.log(`✓ "${tc.phrase}" → ${books[res.bookIndex].name} ${res.chapter}:${res.startVerse} (${res.matchType})`);
  }

  console.log(`\n🎉 All ${passed}/${testPhrases.length} context jump test cases passed!`);
}
