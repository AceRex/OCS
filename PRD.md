...
## Changelog from v1.15 → v1.16 (Reconciliation: Admin Notification Feed + FR Numbering Collision Resolved)

An audit found the actual PRD.md in the repository had diverged from this maintained lineage — it had been directly edited (most likely by the build agent, writing FR numbers straight into the file rather than only proposing them back for merging), reached its own "v1.16," renumbered Live Control to FR-24.1–24.13 under a new Section 4.22, and assigned FR-16.1–16.5 to an entirely different, previously undocumented feature: a real-time admin notification system.

Resolution, per direct confirmation from the project owner: this maintained lineage remains the canonical source of truth. Live Control keeps its original FR-16.1–16.9 numbering exactly as specified in v1.14/v1.15 — it is not renumbered. The notification feed feature is real and is formalized below under new numbers (FR-17.1–17.5) that do not collide with anything already assigned. The repository's PRD.md should be replaced with this file, not reconciled the other direction.

New Section 4.15 (Admin Notification Feed): Formalized from a brief description only ("the admin notification to check") — several specifics are marked NEEDS CONFIRMATION below and should be filled in from the actual implementation in a future audit, following the same practice used previously for the subscription tier system before its details were extracted from source.

Standing process note: build agents should propose new FR numbers in their reports for review and merging, not write them directly into PRD.md. If this recurs, it will keep causing exactly this kind of numbering collision.

---

## Changelog from v1.14 → v1.15 (Live Control Phasing Correction: Transitions Merged Into Phase A)

Correction, not a new decision by the project owner: When PRD v1.14 was originally drafted, the transition library (fade, wipe, T-bar mix) was unilaterally split out into a deferred "Phase B" during documentation — despite the project owner having already specified, in the original design discussion, that the full transition library was wanted in the first version ("Full transition library — cut, fade, wipe, and more, closer to OBS"). That phasing split was never actually confirmed with the project owner; it was introduced while writing the PRD text itself.

An audit subsequently found a complete, tested, live-wired transition engine (fade, 4-direction wipe, manual T-bar, easing curves) already built and flagged it as unauthorized Phase B scope, per the then-current PRD. On review, the project owner confirmed the transition engine should be kept and the PRD corrected to match the original intent, rather than the implementation being rolled back to satisfy a documentation error.

FR-16.7 revised, FR-16.8/16.9 reclassified from Phase B to Phase A below — this is a genuine phasing correction, not scope creep, since the underlying request was present from the original specification. Phase C (RTMP/external streaming, FR-16.10–16.12) remains correctly deferred and untouched — that phasing was never in dispute and still stands, pending its own security review.

---

## Changelog from v1.13 → v1.14 (Live Control — Multi-Camera Video Switcher)

New Section 4.14 (Live Control): Specified before implementation, per the standing spec-checkpoint discipline. Up to 6 paired phones act as camera sources feeding a video-switcher-style Program/Preview workflow, controllable from both Desktop and a permission-designated phone, with transitions and configurable output routing (sanctuary screens and/or external live-streaming platforms). This is the single largest feature specified in this PRD's history and is deliberately phased across three builds — Phase A (core switcher), Phase B (transition library), Phase C (external RTMP streaming) — because Phase C introduces the app's first genuine public-internet-facing capability, a different risk class from anything built before it, including NDI (which is LAN-only).
...
- FR-3.36 restricted the phone's push-to-talk input to "worship leader's own voice commands" without saying which command domains that covers. Given the new presentation/teleprompter commands, this was ambiguous. FR-3.40 explicitly extends secondary input to presentation and teleprompter commands, not just scripture/timer.
...
- Corrected all remaining "Whisper" references — the ASR engine is Vosk throughout; Whisper/WASM is now scoped strictly as a documented fallback path, not the primary description. (Superseded by v1.6's architecture note and this revision's FR-3.65–3.68 — see above.)
...
*(Unchanged from v1.6 — see Section 2.1–2.4: The Overwhelmed Media Operator, The Worship Leader, The Senior Pastor, The Church Tech Lead)*
...
*(Unchanged from v1.6 — React Native/Expo, TypeScript, NativeWind, Redux Toolkit, Socket.IO Client with per-device token — see FR-6.11, expo-av push-to-talk, expo-haptics)*
...
*(Unchanged from v1.6 — FR-1.1 through FR-1.6)*

### 4.2 Bible Controller
*(Unchanged from v1.6 — FR-2.1 through FR-2.8)*
...
- **FR-4.26 (New)** — Text Layer Voice/Quick Entry: A Text layer can be added via the Controller's quick-add panel or a voice command ("OCS add text layer" opens a text-entry prompt in the Controller — text content itself is typed, not spoken, since free-form dictation-to-display is out of scope for accuracy reasons).
...
| :--- | :--- |
| Next slide/media | "OCS next slide" / "OCS next" (in presentation context) |
| Previous slide/media | "OCS previous slide" / "OCS go back" |
| Jump to slide N | "OCS go to slide five" |
| First / last slide | "OCS first slide" / "OCS last slide" |
| Play current media | "OCS play" |
| Pause current media | "OCS pause" |
| Black screen | "OCS black screen" (shared with FR-3.14) |
...
*(No changes. Confirmed intact per this revision's brief — FR-5.1 through FR-5.8 carried forward exactly as in v1.6.)*

#### 4.5.1 Session Folders
*(No changes. FR-5.9 through FR-5.29 carried forward exactly as in v1.6.)*
...
- **FR-14.5 (New)** — Two Install Paths:
  1. "Update Now" — an explicit user action that quits and relaunches the app immediately at the new version.
  2. Defer to natural restart — if the user takes no action, the update is NOT force-installed. It applies automatically the next time the user closes and reopens the app on their own, per `electron-updater`'s standard `autoInstallOnAppQuit` behavior. There is no third path where the app auto-restarts itself unprompted — see FR-14.6.
...
### 4.14 Live Control — Multi-Camera Video Switcher (New in v1.14)
Broadcast-production-style camera switching using paired phones as camera sources. Uses standard vision-mixer terminology throughout: Program (the currently live source), Preview/Multiview (the monitoring grid), Destinations (where Program output is actually sent).

#### Phase A — Core Switcher (this build)
- **FR-16.1 (New)** — Camera Source Pairing: Up to 6 already-paired mobile devices may be designated as camera sources for Live Control, reusing the existing pairing/token infrastructure (FR-6.1–6.13) — no separate camera-specific pairing mechanism.
- **FR-16.2 (New)** — Video Controller Permission: Exactly one entity at a time — either the Desktop or one specific connected phone — holds switching authority ("video controller access"). All other connected camera phones are pure sources with no switching UI available to them. This permission is explicitly grantable/revocable by the Desktop operator, not self-assigned by a phone.
- **FR-16.3 (New)** — Dual-Surface Switcher UI: The Live Control screen is available on both Desktop and whichever phone currently holds video-controller permission (FR-16.2), with equivalent functionality on both — an operator can walk away from the desk with a controller phone and keep switching.
- **FR-16.4 (New)** — Multiview Grid: An 8-tile monitoring grid: up to 6 live camera preview tiles, one General View output monitor tile, and one Speaker View output monitor tile — the monitor tiles show exactly what those screens currently display, not a separate feed, so the operator can visually confirm actual sanctuary output at a glance.
- **FR-16.5 (New)** — Program Selection: Clicking a camera tile makes it the Program (live) source. This is independent of Destination routing (FR-16.6) — selecting a Program source does not by itself determine where it's shown.
- **FR-16.6 (New)** — Destination Routing: The operator independently selects where the current Program feed is sent: General View, Speaker View, or both simultaneously. Multiple destinations may be active at once. External platform destinations (Facebook Live, YouTube Live, etc.) are Phase C scope (FR-16.10+), not part of this phase.
- **FR-16.7 (Revised in v1.15)** — Full Transition Support (Phase A): Switching Program sources supports, at minimum: hard cut, fade/dissolve, and directional wipe, plus a manual T-bar for operator-controlled mix speed and configurable auto-transition duration. This is Phase A scope, not deferred — the earlier v1.14 split that pushed transitions to a separate "Phase B" was a documentation error made while drafting the PRD, not a decision the project owner actually made; the original specification called for the full transition library in the first version.
- **FR-16.8 (Reclassified to Phase A in v1.15)** — Transitions: A transition library comparable to standard broadcast switchers — cut, fade/dissolve, wipe (multi-directional), with configurable easing curves — selectable per-switch by the operator, applied to the Program output when changing camera sources. Implemented and verified live-wired into the Program output rendering path, not a standalone/dormant module.
- **FR-16.9 (Reclassified to Phase A in v1.15)** — Text/Graphic Overlay During Transition: Optional lower-third or text overlay capability during a transition, reusing the existing Text-layer infrastructure (FR-4.25) rather than a parallel implementation, if built as part of this scope — confirm current implementation status against this requirement in the next audit, since v1.14's phasing error may mean this specific piece was deprioritized relative to the core transition engine and needs its own status check.

#### Phase B — Reserved (no current scope)
With transitions moved into Phase A, there is currently no defined Phase B scope for Live Control. Any further capability beyond FR-16.1–16.9 should get its own spec checkpoint before implementation, per the standing rule, rather than being assumed to belong to a reserved "Phase B" slot.

#### Phase C — External Live Streaming (future build, distinct risk class)
- **FR-16.10 (New)** — RTMP Destination Support: The operator may add one or more external RTMP destinations (e.g. Facebook Live, YouTube Live) as additional Destination options (FR-16.6), encoded and streamed via the existing ffmpeg dependency already trusted elsewhere in this app (session bumpers, teleprompter post-processing) rather than a new streaming library.
- **FR-16.11 (New)** — Stream Key Security: RTMP stream keys are credentials of equivalent sensitivity to a password and SHALL be stored using the same OS-native credential store already used for auth tokens (FR-13.4) — never in plaintext config, never logged, never displayed in full after initial entry.
- **FR-16.12 (New)** — First Genuine Public-Internet-Facing Feature: Unlike NDI (FR-4.41–4.45, LAN-only by design) or the auth system (FR-13.x, a controlled login flow), Phase C sends live video to the public internet. This is a materially different risk class and requires its own explicit security review before implementation — specifically around what happens on stream failure/reconnection, and ensuring a failed external stream never silently also disrupts the sanctuary-screen destinations (FR-16.6), which must remain fully independent and unaffected by external streaming health.

### 4.15 Admin Notification Feed (New in v1.16, formalized from brief description)
Real-time notifications within the admin console, alerting platform staff to events needing attention (new tickets, new downloads, or similar) without requiring a manual page refresh. Formalized here from a one-line description only — several specifics below are marked NEEDS CONFIRMATION and should be extracted directly from the actual implementation in the next audit, the same practice used previously for the subscription tier system before its details were pulled from source.

- **FR-17.1 (New)** — Real-Time Delivery Mechanism: Admin console notifications are delivered via WebSocket (not polling), consistent with the "WebSocket Notification Feed" naming found in the repository's divergent PRD copy. NEEDS CONFIRMATION: the actual transport implementation — whether this reuses the existing Socket.IO infrastructure already used for mobile pairing and Live Control, or is a separate WebSocket connection.
- **FR-17.2 (New)** — Notification Triggers: NEEDS CONFIRMATION: the complete, authoritative list of what actually triggers a notification. Plausible candidates based on existing backend features: a new ticket submitted (FR-15.2 already sends an email for this — confirm whether it also triggers an in-console notification), a new download logged, a new testimonial submitted for review. Confirm against actual source rather than assuming all of these fire.
- **FR-17.3 (New)** — Notification Scope and Access: Notifications SHALL only be visible to platform staff (`admin` or `super_admin` roles, per FR-13.15) — never to `church_admin` or `user` accounts. NEEDS CONFIRMATION: this access boundary is actually enforced server-side, not just hidden client-side, consistent with how every other admin-facing surface in this system has needed that same real-server-side check.
- **FR-17.4 (New)** — Notification Persistence and Read State: NEEDS CONFIRMATION: whether notifications persist across sessions (stored, with a read/unread state) or are ephemeral (only visible while connected, lost on refresh).
- **FR-17.5 (New)** — Relationship to Existing Downloads API: The repository's divergent PRD copy grouped this feature together with a "Downloads API" under the same section. NEEDS CONFIRMATION: whether this refers to the existing FR-6.18–6.22 download-tracking/analytics work already specified, or a separate downloads-related capability not yet documented anywhere in this lineage.
...
*(Unchanged from v1.6 — FR-9.1 through FR-9.6.)*

### 5.2 Verse Setlist / Queue
*(Unchanged from v1.6 — FR-10.1 through FR-10.2.)*

### 5.3 Crash Recovery & Session Persistence
*(Unchanged from v1.6 — FR-11.1 through FR-11.4.)*

### 5.4 First-Run Setup Wizard
*(Unchanged from v1.6 — FR-12.1, Steps 1–8. Step 3's "Speak normally" calibration should be re-run whenever the active engine changes per new FR-3.68 — cross-referenced, no wizard step renumbering needed.)*
...
- **FR-5.35** — Teleprompter Voice Commands: "OCS start teleprompter" (loads the currently selected Order of Service note or last-pasted script), "OCS stop teleprompter", "OCS restart teleprompter" (resets cursor to position 0). Subject to FR-3.40 (available from secondary input too).
...
### 6.1 Performance
*(Unchanged from v1.6 — NFR-1 through NFR-8. NFR-1's <3s target applies regardless of active engine; whisper.cpp's chunk-based finals must still land within this budget — flagged as a soak-test item in the Roadmap.)*

### 6.2 Reliability
*(Unchanged from v1.6 — NFR-9 through NFR-15.)*

### 6.3 Compatibility
*(Unchanged from v1.6 — NFR-16 through NFR-19.)*
...
### 6.4 Disk & Memory
*(Unchanged from v1.6 — NFR-20 through NFR-24, still flagged "re-baseline" pending bench harness results.)*

### 6.5 Security & Privacy
*(NFR-25, NFR-27 unchanged from v1.6.)*
...
### 6.6 Accessibility
*(Unchanged from v1.6 — NFR-29 through NFR-32.)*

### 6.7 Session Archive
*(Unchanged from v1.6 — NFR-33 through NFR-36. Timer Controller and Session Folders confirmed untouched in this revision.)*

---

## 7. Success Metrics
*(Unchanged from v1.6 — Sections 7.1–7.4, with one addition below.)*
...
## 8. Risks & Mitigations
*(All v1.6 rows carried forward unchanged. New rows below.)*
...
## 11. Glossary
*(All v1.6 terms carried forward. New terms below.)*
